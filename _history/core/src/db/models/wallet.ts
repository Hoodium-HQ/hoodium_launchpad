...
const WalletSchema = new Schema(
  {
    ...
    /**
     * `AL-8.1` — opt-in to managed LP, and the only thing that starts a fee
     * accrual.
     *
     * Default off, and deliberately not implied by anything else. Registering an
     * address is a public watch-only act (`AL-1.1`); connecting a wallet, being
     * monitored, and receiving alerts all stay free. A user who never touches
     * this field is never billed, and there is no path that sets it on their
     * behalf.
     *
     * Enrolling is still not authority over funds (`AL-8.8`) — settlement runs
     * through the same session-key allowlist as every other action (`AL-N1`).
     */
    autoLp: {
      enrolledAt: { type: Date, default: null },
      unenrolledAt: { type: Date, default: null },
      /** The rate quoted and agreed at enrolment, mirrored onto each ledger. */
      feeRateBps: { type: Number, default: null, min: 0, max: 10_000 },
    },
