...
const EnvSchema = z
  .object({
    ...
    /**
     * Ethereum mainnet endpoint used only to reverse-resolve ENS names.
     *
     * Off unless set, and no default: choosing an endpoint on an operator's
     * behalf would quietly choose which third party gets to see their users'
     * addresses. That is the whole reason this lookup happens here rather than
     * in the browser — the web app disables ENS deliberately so that no
     * third-party origin is contacted from a page where users sign.
     *
     * Read-only and never signed against, so it sits outside the mainnet guard
     * for the same reason `MARKET_RPC_URL` does.
     */
    ...
    // ── Performance fee (001 R8 · design section 6a) ──────────────────────────

    /**
     * Off by default, and that is not a placeholder.
     *
     * A deployment that has not set a fee vault must not quietly start recording
     * amounts owed to nobody. Turning this on is a deliberate act, and `AL-8.7`'s
     * checks below fire the moment it is.
     */
    AUTO_LP_FEE_ENABLED: boolish.default(false),
    /**
     * The platform's share of fees earned by an enrolled position, in basis
     * points. `R8`'s table: 10%.
     *
     * Captured onto each ledger at enrolment (`AL-8.4`), so changing this
     * re-prices future enrolments only — it can never re-price fees that have
     * already been earned.
     */
    AUTO_LP_FEE_BPS: z.coerce.number().int().min(0).max(10_000).default(1_000),
    /**
     * `AL-8.7` — the ceiling a deployment may not cross, mirroring
     * `MAX_PROTOCOL_FEE_SHARE_BPS` in 002's `LPLocker`.
     *
     * That one is enforced in a constructor and is therefore immutable; this one
     * is an environment variable and cannot be. It is here so the intent is
     * checked at boot rather than existing only in a document, and it is the
     * thing to move into a contract when settlement (T7.8) is built.
     */
    AUTO_LP_FEE_MAX_BPS: z.coerce.number().int().min(0).max(10_000).default(3_000),
    /**
     * Where settled fees go. `AL-8.7`, matching 002's `LP-3.5`: a multisig, never
     * an EOA.
     *
     * Nothing can verify multisig-ness from an address alone, so this is not a
     * check — it is a required, named value, so that choosing the destination is
     * a decision someone makes rather than a default they inherit.
     */
    AUTO_LP_FEE_VAULT: z
      .string()
      .regex(/^0x[0-9a-fA-F]{40}$/)
  ...
  .superRefine((env, ctx) => {
    ...
    /*
     * AL-8.7. Both halves fire only when the fee is switched on, so a deployment
     * that does not charge one never has to name a vault.
     *
     * The ceiling is checked against the configured maximum rather than a
     * literal so that lowering the maximum is itself a valid tightening — an
     * operator who wants a 5% cap sets AUTO_LP_FEE_MAX_BPS=500 and the boot
     * refuses anything above it from then on.
     */
    if (env.AUTO_LP_FEE_ENABLED && env.AUTO_LP_FEE_BPS > env.AUTO_LP_FEE_MAX_BPS) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['AUTO_LP_FEE_BPS'],
        message:
          `AUTO_LP_FEE_BPS=${env.AUTO_LP_FEE_BPS} exceeds AUTO_LP_FEE_MAX_BPS=${env.AUTO_LP_FEE_MAX_BPS} (AL-8.7). ` +
          'A performance fee above the stated ceiling is a different product than the one the spec priced.',
      })
    }
    if (env.AUTO_LP_FEE_ENABLED && !env.AUTO_LP_FEE_VAULT) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['AUTO_LP_FEE_VAULT'],
        message:
          'AUTO_LP_FEE_VAULT is required when AUTO_LP_FEE_ENABLED=true (AL-8.7, mirroring 002 LP-3.5). ' +
          'Accruing a fee with no destination records a debt owed to nobody.',
      })
    }
