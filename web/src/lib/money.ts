/**
 * Money primitives — vendored from the retired `@hoodium/shared/money` module.
 *
 * "Monetary values SHALL use string/bigint end to end; JS `Number` for money
 *  SHALL be rejected in review."
 *
 * `Money` is `string | bigint`. Passing a `number` is a compile error — a rule
 * that lives in a prop type does not get disabled the way a lint rule does. The
 * `money/no-number-on-money` ESLint rule is the second line.
 */
...
export class NotMoneyError extends TypeError {
  constructor(received: unknown) {
    super(
      `expected a decimal string or bigint, received ${typeof received} (${String(received)}). ` +
...
export interface FormatOptions {
  ...
  prefix?: string
  suffix?: string
  /** Always show a leading + or −. Sign pairs with colour, never colour alone. */
  signed?: boolean
}
...
export function formatAmount(value: Money, options: FormatOptions = {}): string {
  ...
  // Compact drops trailing zeros — "$29.7k", not "$29.70k". Full precision
  // keeps them: "1,200.00 USDG" reading as "1,200" loses the cents a balance is
  // stated to.
...
/** Exact addition. The alternative is `Number(a) + Number(b)`, which is banned. */
...
/** Direction is never carried by colour alone; pair this with a glyph. */
...
/**
 * A quote-denominated price that may be tiny, formatted so it never collapses
 * to "$0.00". Launchpad tokens open around 1e-6 USDG; two decimals would print
 * every one of them as zero. Keeps four significant digits once the value drops
 * below one.
 */
export function formatPrice(value: Money, prefix = ''): string {
  const parts = decompose(value)
  if (isZeroDigits(parts.int, parts.frac)) return `${prefix}0`
  if (parts.int !== '0') return formatAmount(value, { dp: parts.int.length >= 4 ? 2 : 4, prefix })

  // Count leading zeros in the fraction, keep four significant digits after them.
  const leading = parts.frac.match(/^0*/)?.[0].length ?? 0
  const dp = Math.min(leading + 4, 18)
  const rounded = round(recompose({ ...parts, neg: false }), dp)
  const trimmed = rounded.replace(/0+$/, '').replace(/\.$/, '')
  return `${parts.neg ? '−' : ''}${prefix}${trimmed || '0'}`
}
